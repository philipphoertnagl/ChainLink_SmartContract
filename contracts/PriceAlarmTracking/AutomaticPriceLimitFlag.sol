...
contract PriceAlertSystem {
    ...
    function addAsset(string memory name, address priceFeed, int256 priceLimit) public {
        require(priceFeed != address(0), "invalide price feed adress");
...
function updateThreshold(string memory name, int256 newThreshold) public {
    ...
    // checking the price here automatically
    AggregatorV3Interface priceFeed = AggregatorV3Interface(assets[name].priceFeed);
    (, int256 price,, uint256 updatedAt,) = priceFeed.latestRoundData();

    if (price >= newThreshold) {
        assets[name].priceLimitReached = true;
